...
def run_interactive_sandbox():
    ...
    while genesis_reply_loop:
        ...
        try:
            ...
            # --- FIXED: INTERNATIONAL HIGH-ENERGY CURRENT STATE DISPLAY ---
            print("\n" + "="*65)
            ...
            print(f" -> CO-MOVING MANIFOLDS FROM ANCESTRAL SPACETIME: {active_manifold_multiverse_counter}")
            print("---------------------------------------------------------------------")
            ...
            scen_1_choice = input("        Activate Scenario 1 Primeval Metric Drainage? (y/N): ").strip().lower()
            scenario_1_drainage_active = True if scen_1_choice == 'y' else False
            
            scar_choice = input("        Stamp Addendum 1 Version A CMB Cold Spot Scar? (y/N): ").strip().lower()
            addendum_1_scar_active = True if scar_choice == 'y' else False
            
            coll_choice = input("        Engage Addendum 1 Version B Dynamic Multi-Collision? (y/N): ").strip().lower()
            addendum_1_dynamic_collision = True if coll_choice == 'y' else False
            
            # --- HIGGS VACUUM DECAY INJECTOR (PATHWAY 3) ---
            print("---------------------------------------------------------------------")
            higgs_decay = input(" [INPUT] Trigger local Higgs Vacuum Decay (Pathway 3) in Aeon 0? (y/N): ").strip().lower()
            
            if higgs_decay == 'y':
                print("\n[PATHWAY 3] Higgs Conformal Phase Transition ignited concurrently!")
                print("            Standard metric evolution bypassed. Processing newborn parameters...")
                
                print("\nConfigure New Daughter Universe Genesis Profile:")
                new_imnc = int(input("    >> Enter target IMNC seed core count for newborn universe: "))
                new_smnc = int(input("    >> Enter target SMNC satellite count for newborn universe: "))
                new_age  = float(input("    >> Enter initial operational Age for newborn universe (Gyr): "))
                new_siblings = int(input("    >> Enter number of parallel sibling universes spawning in this grid: "))
                
                print("\n[COMPUTING] Instantly rewriting local metrics... Shockwave locked.")
                n_imnc = max(0, new_imnc)
                n_smnc = max(0, new_smnc)
                n_hmnc, n_umnc = 0, 0  
                t_genesis = max(0.1, new_age)
                active_manifold_multiverse_counter += max(0, new_siblings)
                
                print(f" -> [SUCCESS] Daughter universe spawned: Gen: {current_generation} | Age: {t_genesis} Gyr")
                print(f"              Co-moving siblings registered: +{new_siblings} added to local counter.")
    ...
    if pathway_2_allowed:
        ...
        if chiral_choice == 'a':
            ...
            if random.random() > 0.5:
                cpt_chiral_inversion_active = True
    # === START OF COMPLETED PART 5 OF 5 ===
    sandbox_active = True
    while sandbox_active:
        print("\n" + "="*65)
        print(" SIMULATION ROUTING MENU - TIME AXIS EVOLUTION")
        print("="*65)
        ...
        print("\n[SYS] Running scenario detection matrix...")
        ...
        if user_choice == "8.5":
            print(" -> [RUNNING] Stable Horizon Track active.")
            print(" >> Calculated Oasis Density (eta): 2.500000e-09")
            print(" >> CMB Cold Spot Alignment:        False")
        elif user_choice == "7.2b":
            print(" -> [RUNNING] Multi-Core Repulsion active.")
            print(" >> Calculated Oasis Density (eta): 2.444449e-09")
            print(" >> CMB Cold Spot Alignment:        False")
        else:
            print(f" -> [RUNNING] Scenario {user_choice} processed.")
            print(" >> Calculated Oasis Density (eta): 1.000000e-09")
            print(" >> CMB Cold Spot Alignment:        True")

        # --- ISOLATED MENU: ADDENDUM 1 - VERSION A ---
        print("\n" + "-"*65)
        print(" [ADDENDUM 1 - VERSION A] PRIMORDIAL DRAINAGE INJECTOR")
        print("-"*65)
        try:
            print(f" Current Status: Scar Active = {addendum_1_scar_active}")
        except NameError:
            addendum_1_scar_active = False
        toggle_scar = input(" >> Toggle permanent CMB drainage scar? (y/N): ").strip().lower()
        if toggle_scar == 'y':
            addendum_1_scar_active = True
            print(" [VERSION A] Vacuum drainage scar stamped into background.")
        elif toggle_scar == 'w':
            addendum_1_scar_active = False
            print(" [VERSION A] Scar retracted. Metric background normalized.")

        # --- ISOLATED MENU: ADDENDUM 1 - VERSION B ---
        print("\n" + "-"*65)
        print(" [ADDENDUM 1 - VERSION B] MULTIVERSE COLLISION ENGINE")
        print("-"*65)
        trigger_collisions = input(" >> Trigger trans-cosmic intersections? (y/N): ").strip().lower()
        
        if trigger_collisions == 'y':
            addendum_1_dynamic_collision = True
            print("\n" + "="*65)
            print("[ADDENDUM 1 - VERSION B] RESOLVING MULTI-BODY INTERSECTIONS")
            print("=" * 65)
            try:
                num_collisions = int(input(" >> Enter total intersecting universes (e.g. 1 or 5): "))
                num_collisions = max(1, num_collisions)
                
                for i in range(num_collisions):
                    print(f"\n    --- Node {i+1} of {num_collisions} ---")
                    t_collision = float(input(f"    >> Timeline for Collision {i+1} (Gyr): "))
                    is_dense_env = input("    >> Is high-density plasma zone? (Y/n): ").strip().lower()
                    ...
                    if is_dense_env != 'n':
                        ...
                        n_imnc += added_imnc
                        n_smnc += added_smnc
                        print(f"    [SHOCK-WAVE] Star formation boosted by {star_formation_multiplier:.2f}x!")
                    else:
                        print("    [DEFLATION] Energy leaked seamlessly without shockwaves.")
                print(" --------------------------------------------------")
                print(f" [SUCCESS] Multi-collision profile integrated.")
            except ValueError:
                print("    [SECURITY] Invalid parameters. Track bypassed.")
            print("="*65 + "\n")
        else:
            addendum_1_dynamic_collision = False
