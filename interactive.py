...
def run_interactive_sandbox():
    ...
    hmnc_evap, umnc_evap, smnc_evap, imnc_evap = 0, 0, 0, 0
    ...
    while genesis_reply_loop:
        ...
        if current_object_count == 0:
            ...
            print("\n" + "-"*50)
            print(" [PATHWAY 2] INDEPENDENT SPACETIME ISOLATION EVALUATOR")
            print("-"*50)
        ...
        print("-"*50)
        chiral_choice = input("        Select Chiral Vector (m/a/R): ").strip().lower()
        if chiral_choice == 'm':
            cpt_chiral_inversion_active = False
            print("           [SECTOR]: Confirmed Standard Matter excess trajectory locked.")
        elif chiral_choice == 'a':
            cpt_chiral_inversion_active = True
            print("           [SECTOR]: CPT-Mirror Inversion triggered! Antimatter sector locked.")
        else:
            cpt_chiral_inversion_active = random.choice([True, False])
            sector_tag = "[A]" if cpt_chiral_inversion_active else "[M]"
            print(f"           [STOCHASTIC ROLL]: Quantum fluctuation selected sector {sector_tag}.")
        ...
        if addendum_1_dynamic_collision:
            ...
            if num_collisions > 0:
                ...
                for t_coll, density_flag in collision_times:
                    ...
                    if is_dense != 'n':
                        star_formation_mod *= random.uniform(2.5, 5.0)
        print("---------------------------------------------------------------------")
        ...
        for slot in range(1, 13):
            ...
            if timeline_displacement_risk: ui_write_chance *= 0.15
            if data.get("replacement_shield", False): ui_write_chance *= 0.50
            ...
            if random.random() <= ui_write_chance:
                ...
                if calculated_delay_gyr == float('inf') or calculated_delay_gyr > ignorance_threshold:
