    ...
    while is_manifold_active:
        ...
        if (cycle % 1000 == 0):
            ...
            if total_active_mass > 0:
                ...
                if n_imnc > 10:
                    imnc_to_smnc = int64(n_imnc * 0.01 * flux_efficiency * (n_imnc / max(1.0, float(total_active_mass))))
                    if imnc_to_smnc > 0:
                        n_imnc -= imnc_to_smnc
                        n_smnc += imnc_to_smnc
                        
                if n_smnc > 5:
                    smnc_to_umnc = int64(n_smnc * 0.005 * flux_efficiency * (n_smnc / max(1.0, float(total_active_mass))))
                    if smnc_to_umnc > 0:
                        n_smnc -= smnc_to_umnc
                        n_umnc += smnc_to_umnc
                        
                if n_umnc > 2:
                    umnc_to_hmnc = int64(n_umnc * 0.001 * flux_efficiency * (n_umnc / max(1.0, float(total_active_mass))))
                    if umnc_to_hmnc > 0:
                        n_umnc -= umnc_to_hmnc
                        n_hmnc += umnc_to_hmnc
        ...
        if primordial_spacetimes > 0:
            ...
            decay_step = int64(n_imnc * (lqg_elastic_recoil * 0.05))
        ...
        if n_imnc > 0 or n_smnc > 0 or n_umnc > 0 or n_hmnc > 0:
            pull_imnc = int64(n_imnc * 0.05) if n_imnc > 0 else int64(0)
            pull_smnc = int64(n_smnc * 0.08) if n_smnc > 0 else int64(0)
            pull_umnc = int64(n_umnc * 0.02) if n_umnc > 0 else int64(0)
            pull_hmnc = int64(n_hmnc * 0.01) if n_hmnc > 0 else int64(0)
            ...
            if (f_shear_eff / a_eff) > sigma_qg:
                ...
                if primordial_spacetimes + generated_nodes <= current_total_cores:
                    ...
                    stolen_hmnc = min(n_hmnc, int64(generated_nodes * 0.1))
                    n_hmnc = max(int64(0), n_hmnc - stolen_hmnc)
        ...
        n_imnc -= min(n_imnc, int64(n_imnc * (1.0 - math.exp(-r_imnc * time_per_cycle * 0.01))))
        n_smnc -= min(n_smnc, int64(n_smnc * (1.0 - math.exp(-r_smnc * time_per_cycle * 0.01))))
        n_umnc -= min(n_umnc, int64(n_umnc * (1.0 - math.exp(-r_umnc * time_per_cycle * 0.001))))
